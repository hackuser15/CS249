...
def predictLabelStanford(test_data, preTrained = False):
    path_stanford = os.getcwd() + '\stanford-ner-2015-12-09'
...
test_data.to_csv('test_data', sep=' ', header=False , index=False)
...
pred_data.to_csv('test1', sep=' ', header=False , index=False)
pred_data = getProductOccurence(pred_data)
print(pred_data)
pred_data.to_csv('test2', sep=' ', header=False , index=False)
